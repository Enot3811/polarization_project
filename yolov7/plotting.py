...
def draw_bboxes(
    ...
    color_1: str = "black",
    color_2: str = "white",
    get_rectangle_corners_fn: Callable = get_rectangle_params_from_pascal_bbox,
):
    ...
    for bbox, label, confidence in zip(bboxes, class_labels, confidences):
        ...
        if label is not None or confidence is not None:
            ...
            if label is not None:
                annotation += f'Class: {label}'
            if confidence is not None:
                annotation += f' Conf: {confidence:.3f}'
            plot_ax.annotate(
                annotation, (rx, ry + height), color=color_2, fontsize=20)
...
def annotate_image(
    ...
    if bboxes:
        draw_bboxes_fn(ax, bboxes=bboxes,
...
def show_image(
    image: np.ndarray, bboxes: Tuple = None, class_labels: List = None,
